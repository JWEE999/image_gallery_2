...
function create_image_slideshow(){
	...
	gridImages.forEach((image, idx) => {
		for (i=0;i<total_images_count;i++){
			...
			img.style.width='0';
			img.onload = function() {
				//if ((img.id==='1')||(img.id==='2')){
				img.style.setProperty('animation', "image_preview_appear_from_small 0.5s forwards");
				img.style.setProperty('-webkit-animation', "image_preview_appear_from_small 1s forwards");
				img.style.animationDelay = (parseInt(img.id)*0.4)+"s";
				img.addEventListener('click', () => open_modal(img.id));
				//}
			}
		}
	})
...
function open_modal(idx){
  ...
  document.getElementById('request_device_orientation_permission').click();
...
mySlides.forEach((image, idx) => {
	...
	image.addEventListener("touchstart", startTouch, {passive: false});
	image.addEventListener("touchend", endTouch, false);
	image.addEventListener("touchmove", moveTouch, {passive: false});
});
...
function endTouch(e) {
	...
	// too slow, not swiping motion
	  ...
	  if (in_preview_mode===true){
		...
		document.getElementById("mySlides_"+slideIndex).style.setProperty('-webkit-animation', "");
		...
		document.getElementById("mySlides_"+slideIndex).style.setProperty('-webkit-animation', "image_preview_move_back_original_location 0.2s forwards");
  ...
  function swipe(){
      ...
      if(endX - initialX < -50){
		  ...
		  document.getElementById("mySlides_"+slideIndex).style.setProperty('-webkit-animation', "image_preview_move_right 0.2s forwards");
	  ...
	  if(endX - initialX > 50){
		  ...
		  document.getElementById("mySlides_"+slideIndex).style.setProperty('-webkit-animation', "image_preview_move_left 0.2s forwards");
  ...
  function plusslide(){
	...
	document.getElementById("mySlides_"+nextSlideIndex).style.setProperty('-webkit-animation', "");
	...
	document.getElementById("mySlides_"+nextSlideIndex).style.setProperty('-webkit-animation', "image_preview_appear_from_small 0.2s forwards");
	plusSlides(1);
  }
        ...
        DeviceOrientationEvent.requestPermission()
            ...
            if ( response == "granted" ) {
                window.addEventListener( "deviceorientation", (e) => {
                    // do nothing first, as we only wanted to get permission
                })
            }
        })
            .catch( console.error )
    } else {
        // mostly only IOS has this DeviceOrientationEvent, Android will hit have DeviceOrientationEvent=undefined which we will do ntg
    }
}
